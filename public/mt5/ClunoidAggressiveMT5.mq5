//+------------------------------------------------------------------+
//|                                        ClunoidAggressiveMT5.mq5    |
//|   Clunoid Trading — AGGRESSIVE MT5 automation (forex + volatility) |
//|                                                                   |
//|   The self-contained, full-power version of the Clunoid general   |
//|   automation. It runs the SAME trend engine that Clunoid's cloud   |
//|   serves for Deriv MT5 — but the whole analysis lives INSIDE this  |
//|   EA, so it needs no internet, no signal feed and no Clunoid       |
//|   account. Every decision is made on YOUR terminal from YOUR       |
//|   broker's own prices, always at the aggressive risk setting.      |
//|                                                                   |
//|   WHAT IT TRADES                                                  |
//|   Major currency pairs and Deriv Volatility indices together, one  |
//|   chart running the whole basket. On a Deriv MT5 account it trades  |
//|   both; on any other broker the Volatility indices simply aren't   |
//|   found and it trades the forex majors it can resolve.            |
//|                                                                   |
//|   HOW IT TRADES (unchanged from the Deriv MT5 automation)          |
//|   A pullback-and-resume trend entry with an ADX gate, a structural |
//|   stop, a risk-to-reward target, ATR position sizing to your       |
//|   balance, partial profit taking, stop trailing, pyramiding into   |
//|   winners and an account-wide open-risk cap. Aggressive only:      |
//|   1% risk per trade, up to 5% total open risk.                    |
//|                                                                   |
//|   Forex is gated to the London/New York overlap (its liquid,       |
//|   edge-bearing hours, enforced in GMT); the 24/7 Volatility        |
//|   indices are exempt and trade around the clock.                  |
//|                                                                   |
...
input group           "=== Markets ==="
input string      InpSymbols         = "EURUSD,GBPUSD,USDJPY,USDCAD,AUDUSD,Volatility 75 Index,Volatility 100 Index,Volatility 50 Index,Volatility 25 Index"; // Forex majors + Volatility indices
...
input long        InpMagic           = 77120600;   // Magic number (this EA's trades only)
...
#define TRAIL_ATR     3.5    // wider than gold/crypto: FX retraces more inside a move
...
void ApplyProfile()
  {
   // Aggressive only — 1% risk per trade, 5% total open risk, up to 2 adds.
   g_riskPct=1.00; g_maxOpenRiskPct=5.0; g_maxAdds=2;
...
string ProfileStr() { return("Aggressive"); }

/**
 * Is the market inside the tradable window? Uses GMT, never server time —
 * brokers run their servers on all sorts of offsets (this one is GMT+3), and a
 * session rule written in server time silently trades the wrong five hours.
 */
// Deriv Volatility indices trade 24/7 — they must NOT be gated by the forex
// London/NY session window. Anything with a spaced index-style name is treated
// as always-on; plain FX pairs get the session gate.
bool Is247(const string sym)
  {
   string u=sym; StringToUpper(u);
   return(StringFind(u,"VOLATILITY")>=0 || StringFind(u,"INDEX")>=0 ||
          StringFind(u,"BOOM")>=0 || StringFind(u,"CRASH")>=0 || StringFind(u,"JUMP")>=0 || StringFind(u,"STEP")>=0);
  }
bool InSession()
  {
   MqlDateTime g;
   ...
   return(h >= InpSessionStartGMT || h <= InpSessionEndGMT);   // window crossing midnight
  }
...
//+------------------------------------------------------------------+
/**
 * A genuine FX pair settles its profit in the quote currency — USDJPY pays in
 * JPY — and the terminal converts to the account currency for us. So we must NOT
 * insist that profit currency equals account currency here; that test belongs to
 * exotic CFD crosses (a BTC/ETH pair really does pay in Ether, and sizing off it
 * would be wrong by the price of Ether). Allow anything the broker calculates as
 * FOREX, and demand same-currency settlement only for everything else.
 */
bool SymbolUsable(const string s)
  {
   if(StringFind(s," ")>=0) return(false);      // synthetics carry spaces in their names
   ...
   long calc = SymbolInfoInteger(s,SYMBOL_TRADE_CALC_MODE);
   if(calc==SYMBOL_CALC_MODE_FOREX || calc==SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE) return(true);
   return(SymbolInfoString(s,SYMBOL_CURRENCY_PROFIT) == AccountInfoString(ACCOUNT_CURRENCY));
  }
...
bool AddSymbol(const string want)
  {
   ...
   if(s=="")
     { PrintFormat("Clunoid Aggressive: '%s' is not tradable on this account — skipping it.", want); return(false); }
   ...
   if(slot.hEmaF==INVALID_HANDLE || slot.hEmaS==INVALID_HANDLE || slot.hAdx==INVALID_HANDLE ||
     ...
     { PrintFormat("Clunoid Aggressive: could not build indicators for %s (err %d)", s, GetLastError()); return(false); }
...
/**
 * Account-wide open risk. The majors are close to a single dollar bet — EURUSD
 * and USDCHF correlate at -0.85, EURUSD and GBPUSD at 0.81, and every pair we
 * measured averages 0.43-0.68 dollar-aligned correlation with the rest. So risk
 * is summed at FULL weight across pairs with no diversification credit.
 */
double OpenRiskPct()
  {
   double basis = MathMin(AccountInfoDouble(ACCOUNT_BALANCE),AccountInfoDouble(ACCOUNT_EQUITY));
...
/** Freeze blocks CLOSING against the position's own SL/TP — never ask this
    about the current price, which is trivially zero away from itself. */
...
bool Succeeded(const string what)
  {
   ...
   if(rc==TRADE_RETCODE_MARKET_CLOSED) return(false);   // normal at the weekend
   PrintFormat("Clunoid Aggressive: %s failed — retcode %u (%s)", what, rc, g_trade.ResultRetcodeDescription());
   return(false);
  }
...
void TryEnter(const int k, const Setup &s)
  {
   ...
   SymSlot t = g_sym[k];
   if(!Is247(t.name) && !InSession()) return;   // FX only trades the overlap; synthetics run 24/7
   ...
   if(note!="") Print("Clunoid Aggressive: ", note);
   if(lots<=0.0) return;

   if(OpenRiskPct() + riskPct > g_maxOpenRiskPct)
     {
      PrintFormat("Clunoid Aggressive: skipping %s — open risk %.2f%% + %.2f%% would pass the %.1f%% account cap (pairs are counted together because the majors move as one dollar bet).",
   ...
   bool sent = (s.dir>0)
               ? g_trade.Buy (lots,t.name,0.0,s.sl,s.tp,"Clunoid Aggressive")
               : g_trade.Sell(lots,t.name,0.0,s.sl,s.tp,"Clunoid Aggressive");
   if(!sent || !Succeeded("entry")) return;

   g_sym[k].lastEntry = TimeCurrent();
   double rr = (s.risk>0.0) ? MathAbs(s.tp-s.price)/s.risk : 0.0;
   PrintFormat("Clunoid Aggressive: %s %.2f %s @ %.*f | SL %.*f | TP %.*f | %.1fR | confidence %.0f%s",
         ...
         if(half>=vmin-tol && (vol-half)>=vmin-tol && !FrozenForClose(t.name,dir,sl,tp))
           {
            ...
            if(g_trade.PositionClosePartial(ticket,half) && Succeeded("partial close"))
               PrintFormat("Clunoid Aggressive: %s — banked %.2f at 1R, letting the rest run.", t.name, half);
           }
            ...
            if(flipped && !FrozenForClose(t.name,dir,sl,tp))
              {
               ...
               if(g_trade.PositionClose(ticket) && Succeeded("flip exit"))
                  PrintFormat("Clunoid Aggressive: %s — trend flipped while green, took the profit.", t.name);
              }
           }
        }
     }
...
void StartupReport()
  {
   ...
   PrintFormat("Clunoid Aggressive EA v1.0 — %d pair(s) | profile %s (%.2f%% per trade, %.1f%% total across ALL pairs) | balance %.2f %s | %s account",
               g_symN, ProfileStr(), g_riskPct, g_maxOpenRiskPct, bal, cur, g_hedging?"hedging":"netting");
   PrintFormat("Clunoid Aggressive: trading window %02d:00-%02d:59 GMT (London/New York overlap) — it is %02d:%02d GMT now, so the window is %s.",
   ...
   if(!g_hedging && InpEnablePyramid)
      Print("Clunoid Aggressive: NETTING account — adding to winners is disabled, because extra orders merge into one position instead of stacking.");
   ...
   for(int i=0;i<g_symN;i++)
     {
      ...
      PrintFormat("Clunoid Aggressive:   %s bid %.*f | contract %.0f | min lot %.2f | spread %.4f%%",
                  t.name, t.digits, tk.bid, cs, vmin,
                  tk.bid>0.0 ? ((tk.ask-tk.bid)/tk.bid)*100.0 : 0.0);
      if(lpl>0.0 && bal>0.0)
        {
         double riskAtMin = lpl*vmin, pct = riskAtMin/bal*100.0;
         PrintFormat("Clunoid Aggressive:     a typical stop costs %.2f %s at the minimum lot = %.2f%% of your balance.",
                     riskAtMin, cur, pct);
         if(pct > g_maxOpenRiskPct)
            PrintFormat("Clunoid Aggressive:     WARNING — that exceeds your %.1f%% cap, so %s setups will mostly be SKIPPED. You would need about %.0f %s.",
...
int OnInit()
  {
   ...
   if(InpSessionStartGMT<0 || InpSessionStartGMT>23 || InpSessionEndGMT<0 || InpSessionEndGMT>23)
     { Print("Clunoid Aggressive: session hours must be 0-23 (GMT)."); return(INIT_FAILED); }
   ...
   if(g_symN==0)
     {
      Print("Clunoid Aggressive: none of the requested pairs are tradable here. Check the Pairs input, or open Market Watch and show your broker's USDJPY.");
      return(INIT_FAILED);
     }
   ...
   Print("Clunoid Aggressive: analysing every pair now...");
   for(int i=0;i<g_symN;i++)
     {
      Setup s = Analyse(i);
      PrintFormat("Clunoid Aggressive:   %-10s %-5s conf %3.0f  %s",
...
/** The chart symbol may not be a pair we trade, so a timer drives everything. */
void OnTimer()
  {
   RollDay();
   for(int i=0;i<g_symN;i++)
     {
      ...
      if(bar==0 || bar==g_sym[i].lastBar) continue;   // closed bars only
      g_sym[i].lastBar = bar;

      ManageSymbol(i);                                 // manage regardless of session
      if(!TradingAllowed()) continue;
      Setup s = Analyse(i);
      if(s.valid) TryEnter(i,s);                       // TryEnter enforces the session
     }
  }
