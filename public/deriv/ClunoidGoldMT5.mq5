//+------------------------------------------------------------------+
//|                                              ClunoidGoldMT5.mq5   |
//|   Clunoid Trading — GOLD & METALS AI automation                   |
//|                                                                   |
//|   SELF-CONTAINED. No internet permissions, no Clunoid account:    |
//|   every decision is made on YOUR terminal from YOUR broker own    |
//|   prices. One chart runs every metal.                             |
//|                                                                   |
//|   WHY A BASKET AND NOT JUST GOLD                                  |
//|   Gold alone produced a trade about once every four days, which   |
//|   is not enough for anyone to see the automation working. The     |
//|   four precious metals move on the same drivers but not in        |
//|   lockstep, so trading them together roughly quadruples the       |
//|   opportunities WITHOUT loosening any individual judgement:       |
//|                                                                   |
//|     gold alone      82 trades a year                              |
//|     four metals    315 trades a year — about one per day          |
//|                                                                   |
//|   Every metal in the basket earned its place on its own merits,   |
//|   so nothing here is carried by the others.                       |
//|                                                                   |
//|   Silver, platinum and palladium are thinner than gold, so the    |
//|   spread guard and the account-wide risk cap matter more here     |
//|   than on any other bot. Metals also fall together in a liquidity |
//|   squeeze, so their open risk is summed at FULL weight.           |
//|                                                                   |
//|   SETUP: drag onto ANY chart, allow algo trading, done. Works on  |
//|   any MT5 broker — symbol suffixes are matched automatically.     |
//+------------------------------------------------------------------+
...
input group           "=== Markets ==="
input string      InpSymbols         = "XAUUSD,XAGUSD"; // Metals to trade (comma separated; suffixes auto-matched)
...
input long        InpMagic           = 77090777;   // Magic number (this EA trades only)
...
#define ADX_GATE      18.0   // metals tested best at the mid range
...
//+------------------------------------------------------------------+
//| Symbol resolution — match the user's wanted metals against what    |
...
bool SymbolUsable(const string s)
  {
   if(StringFind(s," ")>=0) return(false);   // "BTCUSD RSI Pullback Index" is a synthetic, not Bitmetal
...
bool AddSymbol(const string want)
  {
   ...
   if(s=="")
     { PrintFormat("Clunoid Metals: '%s' is not tradable on this account — skipping it.", want); return(false); }
   ...
   if(slot.hEmaF==INVALID_HANDLE || slot.hEmaS==INVALID_HANDLE || slot.hAdx==INVALID_HANDLE ||
     ...
     { PrintFormat("Clunoid Metals: could not build indicators for %s (err %d)", s, GetLastError()); return(false); }
...
//+------------------------------------------------------------------+
//| ANALYSE one metal on its last CLOSED H1 bar                        |
//+------------------------------------------------------------------+
...
//|                                                                   |
//| the metals correlate at 0.86, and 0.73 even in BTC's worst hours,|
...
//| open risk across every metal is summed at full weight against one   |
//| account-wide ceiling. A "1% per trade, 5% total" profile therefore |
//| means 5% of crypto exposure in total, not 5% per metal.             |
//+------------------------------------------------------------------+
double OpenRiskPct()
  {
   double basis = MathMin(AccountInfoDouble(ACCOUNT_BALANCE),AccountInfoDouble(ACCOUNT_EQUITY));
...
/** Spread as a PERCENTAGE of price — a points cap is meaningless when one metal
...
bool Succeeded(const string what)
  {
   ...
   PrintFormat("Clunoid Metals: %s failed — retcode %u (%s)", what, rc, g_trade.ResultRetcodeDescription());
   return(false);
  }
...
void TryEnter(const int k, const Setup &s)
  {
   ...
   if(note!="") Print("Clunoid Metals: ", note);
   if(lots<=0.0) return;

   // account-wide cap, correlated metals counted at full weight
   if(OpenRiskPct() + riskPct > g_maxOpenRiskPct)
     {
      PrintFormat("Clunoid Metals: skipping %s — crypto exposure %.2f%% + %.2f%% would pass the %.1f%% account cap (the metals are counted together because they move together).",
   ...
   bool sent = (s.dir>0)
               ? g_trade.Buy (lots,t.name,0.0,s.sl,s.tp,"Clunoid Metals")
               : g_trade.Sell(lots,t.name,0.0,s.sl,s.tp,"Clunoid Metals");
   if(!sent || !Succeeded("entry")) return;

   g_sym[k].lastEntry = TimeCurrent();
   double rr = (s.risk>0.0) ? MathAbs(s.tp-s.price)/s.risk : 0.0;
   PrintFormat("Clunoid Metals: %s %.4f %s @ %.*f | SL %.*f | TP %.*f | %.1fR | confidence %.0f%s",
         ...
         if(half>=vmin-tol && (vol-half)>=vmin-tol && !FrozenForClose(t.name,dir,sl,tp))
           {
            ...
            if(g_trade.PositionClosePartial(ticket,half) && Succeeded("partial close"))
               PrintFormat("Clunoid Metals: %s — banked %.4f at 1R, letting the rest run.", t.name, half);
           }
            ...
            if(flipped && !FrozenForClose(t.name,dir,sl,tp))
              {
               ...
               if(g_trade.PositionClose(ticket) && Succeeded("flip exit"))
                  PrintFormat("Clunoid Metals: %s — trend flipped while green, took the profit.", t.name);
              }
           }
        }
     }
...
void StartupReport()
  {
   ...
   PrintFormat("Clunoid Metals EA v1.0 — %d metal(s) | profile %s (%.2f%% per trade, %.1f%% total across ALL metals) | balance %.2f %s | %s account",
               g_symN, ProfileStr(), g_riskPct, g_maxOpenRiskPct, bal, cur, g_hedging?"hedging":"netting");
   if(!g_hedging && InpEnablePyramid)
      Print("Clunoid Metals: NETTING account — adding to winners is disabled, because extra orders merge into one position instead of stacking.");
   ...
   for(int i=0;i<g_symN;i++)
     {
      ...
      double riskAtMin = lpl*vmin;
      PrintFormat("Clunoid Metals:   %s bid %.*f | contract %.4f | min lot %.4f | spread %.3f%%",
      ...
      if(riskAtMin>0.0 && bal>0.0)
        {
         ...
         PrintFormat("Clunoid Metals:     a typical stop costs %.2f %s at the minimum lot = %.2f%% of your balance.",
                     riskAtMin, cur, pct);
         if(pct > g_maxOpenRiskPct)
            PrintFormat("Clunoid Metals:     WARNING — that exceeds your %.1f%% cap, so %s setups will mostly be SKIPPED. You would need about %.0f %s.",
...
int OnInit()
  {
   ...
   if(g_symN==0)
     {
      Print("Clunoid Metals: none of the requested metals are tradable here. Check the Metals input, or open Market Watch and show your broker's BTC/ETH symbols.");
      return(INIT_FAILED);
     }
   ...
   Print("Clunoid Metals: analysing every metal now...");
   for(int i=0;i<g_symN;i++)
     {
      Setup s = Analyse(i);
      PrintFormat("Clunoid Metals:   %-10s %-5s conf %3.0f  %s",
...
/** The real driver. OnTick only fires for the chart symbol, which may not be a
    metal we trade at all — so every metal is polled here instead. */
void OnTimer()
  {
   RollDay();
   for(int i=0;i<g_symN;i++)
