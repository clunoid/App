//+------------------------------------------------------------------+
//|                                             ClunoidForexMT5.mq5   |
//|   Clunoid Trading — MAJOR FOREX AI automation (USD/JPY focused)   |
//|                                                                   |
//|   SELF-CONTAINED. No internet permissions, no Clunoid account:    |
//|   every decision is made on YOUR terminal from YOUR broker's own  |
//|   prices. One chart runs every pair you enable.                   |
//|                                                                   |
//|   WHY THIS BOT TRADES USD/JPY AND NOT "THE MAJORS"                |
//|   We tested all seven majors on a year of real Deriv data, the    |
//|   same strategy and the same 128-configuration grid on each, and  |
//|   scored every candidate on BOTH halves of the year separately.   |
//|   The result was not close:                                       |
//|                                                                   |
//|     USDJPY  profit factor 2.09   halves 2.37 / 2.00   <-- ships   |
//|     USDCHF  1.43                 halves 1.51 / 1.35              |
//|     USDCAD  1.21                 halves 1.09 / 1.31              |
//|     AUDUSD  0.98    EURUSD 0.95    GBPUSD 0.73    NZDUSD 0.33     |
//|                                                                   |
//|   For every pair except USD/JPY, not one configuration in the     |
//|   whole grid held up across both halves. So this bot trades       |
//|   USD/JPY by default. The others are switchable but they did not  |
//|   earn a place, and the EA says so rather than quietly trading    |
//|   markets that failed.                                            |
//|                                                                   |
//|   THE SESSION IS THE WHOLE EDGE                                   |
//|   Restricted to the London/New York overlap, 14 of 14 tested      |
//|   configurations were robust (median profit factor 1.92). Allowed |
//|   to trade around the clock, 0 of 48 were, and the median lost    |
//|   money at 0.83. That is not a tuned parameter — it is the        |
//|   difference between trading when the pair is liquid and trading  |
//|   when it is drifting. The window is enforced in GMT so it is     |
//|   correct whatever time zone your broker's server runs on.        |
//|                                                                   |
...
input long        InpMagic           = 77090999;   // Magic number (this EA's trades only)
...
bool AddSymbol(const string want)
  {
   ...
   if(s=="")
     { PrintFormat("Clunoid Forex: '%s' is not tradable on this account — skipping it.", want); return(false); }
   ...
   if(slot.hEmaF==INVALID_HANDLE || slot.hEmaS==INVALID_HANDLE || slot.hAdx==INVALID_HANDLE ||
     ...
     { PrintFormat("Clunoid Forex: could not build indicators for %s (err %d)", s, GetLastError()); return(false); }
...
bool Succeeded(const string what)
  {
   ...
   PrintFormat("Clunoid Forex: %s failed — retcode %u (%s)", what, rc, g_trade.ResultRetcodeDescription());
   return(false);
  }
...
void TryEnter(const int k, const Setup &s)
  {
   ...
   if(!InSession()) return;                       // the edge only exists in the overlap
   SymSlot t = g_sym[k];
   ...
   if(note!="") Print("Clunoid Forex: ", note);
   if(lots<=0.0) return;

   if(OpenRiskPct() + riskPct > g_maxOpenRiskPct)
     {
      PrintFormat("Clunoid Forex: skipping %s — open risk %.2f%% + %.2f%% would pass the %.1f%% account cap (pairs are counted together because the majors move as one dollar bet).",
   ...
   bool sent = (s.dir>0)
               ? g_trade.Buy (lots,t.name,0.0,s.sl,s.tp,"Clunoid Forex")
               : g_trade.Sell(lots,t.name,0.0,s.sl,s.tp,"Clunoid Forex");
   if(!sent || !Succeeded("entry")) return;

   g_sym[k].lastEntry = TimeCurrent();
   double rr = (s.risk>0.0) ? MathAbs(s.tp-s.price)/s.risk : 0.0;
   PrintFormat("Clunoid Forex: %s %.2f %s @ %.*f | SL %.*f | TP %.*f | %.1fR | confidence %.0f%s",
         ...
         if(half>=vmin-tol && (vol-half)>=vmin-tol && !FrozenForClose(t.name,dir,sl,tp))
           {
            ...
            if(g_trade.PositionClosePartial(ticket,half) && Succeeded("partial close"))
               PrintFormat("Clunoid Forex: %s — banked %.2f at 1R, letting the rest run.", t.name, half);
           }
            ...
            if(flipped && !FrozenForClose(t.name,dir,sl,tp))
              {
               ...
               if(g_trade.PositionClose(ticket) && Succeeded("flip exit"))
                  PrintFormat("Clunoid Forex: %s — trend flipped while green, took the profit.", t.name);
              }
           }
        }
     }
...
void StartupReport()
  {
   ...
   PrintFormat("Clunoid Forex EA v1.0 — %d pair(s) | profile %s (%.2f%% per trade, %.1f%% total across ALL pairs) | balance %.2f %s | %s account",
               g_symN, ProfileStr(), g_riskPct, g_maxOpenRiskPct, bal, cur, g_hedging?"hedging":"netting");
   PrintFormat("Clunoid Forex: trading window %02d:00-%02d:59 GMT (London/New York overlap) — it is %02d:%02d GMT now, so the window is %s.",
   ...
   if(!g_hedging && InpEnablePyramid)
      Print("Clunoid Forex: NETTING account — adding to winners is disabled, because extra orders merge into one position instead of stacking.");
   ...
   for(int i=0;i<g_symN;i++)
     {
      ...
      PrintFormat("Clunoid Forex:   %s bid %.*f | contract %.0f | min lot %.2f | spread %.4f%%",
                  t.name, t.digits, tk.bid, cs, vmin,
                  tk.bid>0.0 ? ((tk.ask-tk.bid)/tk.bid)*100.0 : 0.0);
      if(lpl>0.0 && bal>0.0)
        {
         double riskAtMin = lpl*vmin, pct = riskAtMin/bal*100.0;
         PrintFormat("Clunoid Forex:     a typical stop costs %.2f %s at the minimum lot = %.2f%% of your balance.",
                     riskAtMin, cur, pct);
         if(pct > g_maxOpenRiskPct)
            PrintFormat("Clunoid Forex:     WARNING — that exceeds your %.1f%% cap, so %s setups will mostly be SKIPPED. You would need about %.0f %s.",
...
int OnInit()
  {
   ...
   if(InpSessionStartGMT<0 || InpSessionStartGMT>23 || InpSessionEndGMT<0 || InpSessionEndGMT>23)
     { Print("Clunoid Forex: session hours must be 0-23 (GMT)."); return(INIT_FAILED); }
   ...
   if(g_symN==0)
     {
      Print("Clunoid Forex: none of the requested pairs are tradable here. Check the Pairs input, or open Market Watch and show your broker's USDJPY.");
      return(INIT_FAILED);
     }
   ...
   Print("Clunoid Forex: analysing every pair now...");
   for(int i=0;i<g_symN;i++)
     {
      Setup s = Analyse(i);
      PrintFormat("Clunoid Forex:   %-10s %-5s conf %3.0f  %s",
