//+------------------------------------------------------------------+
//|                                            ClunoidCryptoMT5.mq5   |
//|   Clunoid Trading — CRYPTO (BTC / ETH) AI automation, 24/7        |
//|                                                                   |
//|   SELF-CONTAINED. No internet permissions, no Clunoid account:    |
//|   every decision is made on YOUR terminal from YOUR broker's own  |
//|   crypto prices. One chart runs every coin.                       |
//|                                                                   |
//|   WHY THIS IS NOT JUST THE GOLD BOT WITH NEW SYMBOLS              |
//|   We measured a year of real Deriv BTC and ETH data before        |
//|   writing a line of this, and it said three things:               |
//|                                                                   |
//|     1. Crypto does NOT trend the way folklore claims. Momentum    |
//|        autocorrelation on BTC/ETH is about zero at 1-24h (it is   |
//|        slightly NEGATIVE at 4h). So a breakout/momentum bot has   |
//|        no edge to harvest — we tested one, it lost money. Trend   |
//|        logic here is a FILTER for standing aside, not a forecast. |
//|     2. Volatility clusters hard and the tails are wild: 0.86% of  |
//|        hours move beyond 4 sigma, where a normal distribution     |
//|        says 0.006%. That is ~140x. Hence ATR-relative stops and   |
//|        a hard refusal to trade setups whose structure is far away.|
//|     3. BTC and ETH move together with correlation 0.86, and 0.73  |
//|        even in BTC's worst hours. Two positions are NOT two bets. |
//|        So the portfolio cap counts them at FULL weight and gives  |
//|        no diversification credit. This is the single most         |
//|        important difference from a single-market bot.             |
//|                                                                   |
//|   Being far more selective was what actually paid: raising the    |
//|   confidence floor lifted the profit factor monotonically. This   |
//|   bot therefore trades rarely and only on its best reads.         |
//|                                                                   |
//|   SETUP: drag onto ANY chart, allow algo trading, done. The chart |
//|   symbol and timeframe are irrelevant — it reads what it needs.   |
//+------------------------------------------------------------------+
...
input group           "=== Markets ==="
input string      InpSymbols         = "BTCUSD,ETHUSD"; // Coins to trade (comma separated; suffixes auto-matched)
input double      InpMaxSpreadPct    = 0.15;       // Skip entries when spread exceeds this % of price
...
input long        InpMagic           = 77090888;   // Magic number (this EA's trades only)

//--- strategy constants — these ARE the validated configuration -------------
#define EMA_FAST      20     // H1 fast EMA
#define EMA_SLOW      50     // H1 slow EMA
#define EMA_BIAS_F    50     // H4 fast EMA
#define EMA_BIAS_S    200    // H4 slow EMA
...
struct SymSlot
  {
   ...
   int             digits;
   double          point;
   int             hEmaF, hEmaS, hAdx, hAtr;     // H1
   int             hBiasF, hBiasS, hBiasAdx;     // H4
   datetime        lastBar;
   datetime        lastEntry;
   bool            firstRead;   // have we managed one real analysis yet?
  };
...
//+------------------------------------------------------------------+
//| Symbol resolution — match the user's wanted coins against what    |
//| the broker actually calls them (suffixes like BTCUSD.r etc).      |
//+------------------------------------------------------------------+
/**
 * A symbol is only usable if it is tradable AND settles in the account's own
 * currency. That second test is not pedantry: this broker also lists BTCETH and
 * BTCLTC, whose profit currency is ETH or LTC. OrderCalcProfit would hand back a
 * number denominated in Ether while the balance is in dollars, and the position
 * would be sized out by a factor of roughly the ETH price. Refuse them outright.
 */
bool SymbolUsable(const string s)
  {
   if(StringFind(s," ")>=0) return(false);   // "BTCUSD RSI Pullback Index" is a synthetic, not Bitcoin
...
bool AddSymbol(const string want)
  {
   ...
   if(s=="")
     { PrintFormat("Clunoid Crypto: '%s' is not tradable on this account — skipping it.", want); return(false); }
   ...
   if(slot.hEmaF==INVALID_HANDLE || slot.hEmaS==INVALID_HANDLE || slot.hAdx==INVALID_HANDLE ||
     ...
     { PrintFormat("Clunoid Crypto: could not build indicators for %s (err %d)", s, GetLastError()); return(false); }
...
//+------------------------------------------------------------------+
//| Data helpers — always per-symbol, never _Point / _Digits          |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| ANALYSE one coin on its last CLOSED H1 bar                        |
//+------------------------------------------------------------------+
Setup Analyse(const int k)
  {
   ...
   // pullback-and-resume. Crypto has no momentum persistence to ride, so we only
   // ever join a trend that has just paused and restarted — never a run in flight.
   bool retraced = false;
   ...
   if(lots<=0.0)
     {
      ...
      if(pctAtMin <= g_maxOpenRiskPct)
        {
         lots = vmin;
         note = StringFormat("%s: min lot %.4f risks %.2f%% (above the %.2f%% target but inside the %.1f%% cap)",
                             sym, vmin, pctAtMin, g_riskPct, g_maxOpenRiskPct);
        }
      else
        {
         note = StringFormat("SKIP %s: smallest lot %.4f risks %.2f %s = %.1f%% of %.2f — over the %.1f%% cap.",
...
//+------------------------------------------------------------------+
//| PORTFOLIO RISK — the crypto-specific part.                        |
//|                                                                   |
//| BTC and ETH correlate at 0.86, and 0.73 even in BTC's worst hours,|
//| so a BTC position and an ETH position are very nearly the same bet|
//| twice. This deliberately gives NO diversification credit: every    |
//| open risk across every coin is summed at full weight against one   |
//| account-wide ceiling. A "1% per trade, 5% total" profile therefore |
//| means 5% of crypto exposure in total, not 5% per coin.             |
//+------------------------------------------------------------------+
double OpenRiskPct()
  {
   double basis = MathMin(AccountInfoDouble(ACCOUNT_BALANCE),AccountInfoDouble(ACCOUNT_EQUITY));
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if(SlotOf(sym)<0) continue;                    // not one of ours
      ...
      if(dist<=0.0) continue;                        // at break-even = no risk left
...
//+------------------------------------------------------------------+
//| Broker level checks (all per-symbol)                              |
//+------------------------------------------------------------------+
...
/** Freeze applies to CLOSING against the position's own SL/TP, not the price itself. */
...
/** Spread as a PERCENTAGE of price — a points cap is meaningless when one coin
    trades near 100,000 and another near 4,000. */
...
bool Succeeded(const string what)
  {
   ...
   PrintFormat("Clunoid Crypto: %s failed — retcode %u (%s)", what, rc, g_trade.ResultRetcodeDescription());
   return(false);
  }
...
void TryEnter(const int k, const Setup &s)
  {
   ...
   if(note!="") Print("Clunoid Crypto: ", note);
   if(lots<=0.0) return;

   // account-wide cap, correlated coins counted at full weight
   if(OpenRiskPct() + riskPct > g_maxOpenRiskPct)
     {
      PrintFormat("Clunoid Crypto: skipping %s — crypto exposure %.2f%% + %.2f%% would pass the %.1f%% account cap (BTC and ETH are counted together because they move together).",
   ...
   bool sent = (s.dir>0)
               ? g_trade.Buy (lots,t.name,0.0,s.sl,s.tp,"Clunoid Crypto")
               : g_trade.Sell(lots,t.name,0.0,s.sl,s.tp,"Clunoid Crypto");
   if(!sent || !Succeeded("entry")) return;

   g_sym[k].lastEntry = TimeCurrent();
   double rr = (s.risk>0.0) ? MathAbs(s.tp-s.price)/s.risk : 0.0;
   PrintFormat("Clunoid Crypto: %s %.4f %s @ %.*f | SL %.*f | TP %.*f | %.1fR | confidence %.0f%s",
               s.dir>0?"BUY":"SELL", lots, t.name, t.digits, s.price, t.digits, s.sl, t.digits, s.tp,
               rr, s.conf, sameDir>0?" (add)":"");
  }

//+------------------------------------------------------------------+
//| MANAGEMENT — closed-bar decisions; the server holds the real      |
//| stop and target continuously, so the account is covered 24/7.     |
//+------------------------------------------------------------------+
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      // 1 · bank a partial at 1R, then remove the risk entirely
         ...
         if(half>=vmin-tol && (vol-half)>=vmin-tol && !FrozenForClose(t.name,dir,sl,tp))
           {
            ...
            if(g_trade.PositionClosePartial(ticket,half) && Succeeded("partial close"))
               PrintFormat("Clunoid Crypto: %s — banked %.4f at 1R, letting the rest run.", t.name, half);
           }
      ...
      // 2 · trail, but only on a meaningful improvement
            ...
            if(flipped && !FrozenForClose(t.name,dir,sl,tp))
              {
               ...
               if(g_trade.PositionClose(ticket) && Succeeded("flip exit"))
                  PrintFormat("Clunoid Crypto: %s — trend flipped while green, took the profit.", t.name);
              }
           }
        }
     }
...
void StartupReport()
  {
   ...
   PrintFormat("Clunoid Crypto EA v1.0 — %d coin(s) | profile %s (%.2f%% per trade, %.1f%% total across ALL coins) | balance %.2f %s | %s account",
               g_symN, ProfileStr(), g_riskPct, g_maxOpenRiskPct, bal, cur, g_hedging?"hedging":"netting");
   if(!g_hedging && InpEnablePyramid)
      Print("Clunoid Crypto: NETTING account — adding to winners is disabled, because extra orders merge into one position instead of stacking.");
   ...
   for(int i=0;i<g_symN;i++)
     {
      ...
      double atrb[];
      double atr = Buf(t.hAtr,0,3,atrb) ? atrb[1] : 0.0;
      double typicalStop = (atr>0.0) ? 1.6*atr : 0.0;
      double lpl = (typicalStop>0.0) ? LossPerLot(t.name,typicalStop) : 0.0;
      double riskAtMin = lpl*vmin;
      PrintFormat("Clunoid Crypto:   %s bid %.*f | contract %.4f | min lot %.4f | spread %.3f%%",
                  t.name, t.digits, tk.bid, cs, vmin,
                  tk.bid>0.0 ? ((tk.ask-tk.bid)/tk.bid)*100.0 : 0.0);
      if(riskAtMin>0.0 && bal>0.0)
        {
         double pct = riskAtMin/bal*100.0;
         PrintFormat("Clunoid Crypto:     a typical stop costs %.2f %s at the minimum lot = %.2f%% of your balance.",
                     riskAtMin, cur, pct);
         if(pct > g_maxOpenRiskPct)
            PrintFormat("Clunoid Crypto:     WARNING — that exceeds your %.1f%% cap, so %s setups will mostly be SKIPPED. You would need about %.0f %s.",
...
int OnInit()
  {
   ...
   g_trade.SetDeviationInPoints(50);   // crypto moves fast; allow a wider fill window

   g_symN = 0;
   // Trim each name individually rather than stripping every space from the list —
   // some instrument names legitimately contain spaces, and blanket-stripping them
   // silently turns one name into another.
   string parts[];
   ...
   if(g_symN==0)
     {
      Print("Clunoid Crypto: none of the requested coins are tradable here. Check the Coins input, or open Market Watch and show your broker's BTC/ETH symbols.");
      return(INIT_FAILED);
     }
   ...
   Print("Clunoid Crypto: analysing every coin now...");
   for(int i=0;i<g_symN;i++)
     {
      Setup s = Analyse(i);
      PrintFormat("Clunoid Crypto:   %-10s %-5s conf %3.0f  %s",
                  g_sym[i].name, s.dir>0?"LONG":s.dir<0?"SHORT":"flat", s.conf, s.why);
      if(s.valid) TryEnter(i,s);
     }

   // Crypto never closes and the chart symbol may not even be one we trade, so a
   // timer — not chart ticks — is what actually drives this EA.
   EventSetTimer(15);
...
/** The real driver. OnTick only fires for the chart symbol, which may not be a
    coin we trade at all — so every coin is polled here instead. */
void OnTimer()
  {
   RollDay();
   for(int i=0;i<g_symN;i++)
     {
      // A freshly attached EA usually has no H4 history yet, and waiting for the
      // next H1 bar would leave the user staring at "history not ready" for up to
      // an hour. So keep retrying the opening read every tick of the timer until
      // one of them actually succeeds, then settle into closed-bar decisions.
