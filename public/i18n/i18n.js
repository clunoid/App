/**
 * MAGIC BOTS LAB — the language layer.
 *
 ...
 */
/* Clunoid build: React-safe. Plain-text leaves are rewritten through their
   existing text node (React keeps a reference to it), and the switcher
   re-mounts itself after a client-side navigation replaces the header. */
(function () {
  "use strict";
  ...
  var norm = function (s) { return s.replace(/<!--[\s\S]*?-->/g, "").replace(/\s+/g, " ").trim(); };
  ...
  function translateElement(el) {
    ...
    if (el.firstElementChild) return false; // markup inside: its text nodes are translated one by one
  ...
  /* A leaf that is only text keeps its text node: React holds a reference to
     it and writes later updates through that reference, so replacing the node
     would leave React talking to a node that is no longer on the page. The
     translation goes into the first text node, any others are blanked, and
     each remembers what it held so the switch back is exact. */
  var decoder = document.createElement("textarea");
  function writePlain(el, out) {
    ...
    for (var n = el.firstChild; n; n = n.nextSibling) {
      ...
      if (!done && n.nodeValue.trim()) { n.nodeValue = text; done = true; }
      else if (done && n.nodeValue.trim()) n.nodeValue = "";
    }
    if (!done) { if (!originals.has(el)) originals.set(el, el.innerHTML); el.textContent = text; }
  }
      ...
      try {
        for (var i = 0; i < muts.length; i++) {
          ...
          if (m.type === "childList") {
            ...
            if (mount && !mount.isConnected) paintSwitch();
            ...
            if (!translateElement(p)) translateTextNodes(p);
  ...
  function paintSwitch() {
    if (mount && !mount.isConnected) mount = null;
    if (!mount) {
      ...
      if (!mount) {
        ...
        if (hdr) { mount.className = "lang-mount lang-mount--header"; hdr.appendChild(mount); }
