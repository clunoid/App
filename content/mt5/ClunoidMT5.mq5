...
//+------------------------------------------------------------------+
//|  PROPRIETARY — © Clunoid. All rights reserved.                   |
//|  This file, its source and the strategy it contains are the      |
//|  exclusive property of Clunoid (https://www.clunoid.com).        |
...
//+------------------------------------------------------------------+
#property copyright   "Clunoid — proprietary, all rights reserved. No analysis, copying or modification permitted."
#property description "Clunoid MT5 EA v7.0 — early-trend scanner (H1/H4 by default, M1–M15 selectable), profit lock, smart exit, profit target + signal feed. Proprietary: licensed to the approved account holder only; no permission for any person or AI system to analyze, copy, modify or redistribute."
...
#define OWNER_NOTICE "Clunoid EA — proprietary, all rights reserved. Licensed to the approved account holder only; no permission for any person or automated system to analyze, copy, modify or redistribute."
...
int OnInit()
  {
   ...
   Print(OWNER_NOTICE);
   PrintFormat("Clunoid MT5 EA v7.0 started — profile=%s · preferred lot %.2f (risk-based when the account cannot carry it) · trend scanner %s · signal feed: forex + Volatility (needs https://www.clunoid.com in Tools > Options > Expert Advisors > WebRequest; without it the trend scanner trades on its own).", ProfileStr(), InpStandardLot, InpTrendScanner ? "ON" : "off");
   ...
   if(!g_isLeader)                                    // the other copy went away: this one takes over
     {
      ...
      PrintFormat("Clunoid: this copy now runs the account (the other copy stopped).");
...
void Poll()
  {
   ...
   Comment(StringFormat("Clunoid MT5 v7.0 · %s · %d signals · %s%s%s · %s",
...
// Proprietary — Clunoid. No permission to analyze, copy or modify (see the notice at the top).
//+------------------------------------------------------------------+
//| Entries                                                          |
//+------------------------------------------------------------------+
void OpenBase(Sig &s)
  {
   ...
   if(ok)
     ...
     {
      PrintFormat("Clunoid order failed %s %s: %d", s.side, sym, g_trade.ResultRetcode());
   ...
   for(int k=0;k<n;k++)
     {
      ...
      if(ok)
        ...
        { GVset("cl_add_"+sym, done+1); Reserve(sym, had+1); g_uncertain = true; return; }
     }
  }
...
ulong BindPlanTo(string sym, Sig &s, double lots, double riskPct)
  {
   ...
   else PrintFormat("Clunoid: opened %s but could not bind plan yet (will retry next poll).", sym);
   return posTk;
  }
...
void DeleteTicketGVs(ulong tk)
  {
   ...
   GlobalVariableDel("cl_pf_"+T);    GlobalVariableDel("cl_risk_"+T); GlobalVariableDel("cl_clu_"+T); GlobalVariableDel("cl_tr_"+T);
   GlobalVariableDel("cl_pk_"+T); GlobalVariableDel("cl_r0_"+T); GlobalVariableDel("cl_tf_"+T); GlobalVariableDel("cl_cm_"+T); GlobalVariableDel("cl_lk_"+T); GlobalVariableDel("cl_lf_"+T);
...
void LockProfit(ulong tk)
  {
   ...
   double pk = GVget("cl_pk_"+T, 0);
   if(p > pk) { pk = p; GVset("cl_pk_"+T, pk); }
   ...
   if((int)GVget("cl_lk_"+T, 0) != stage)
     {
      GVset("cl_lk_"+T, stage);
      PrintFormat("Clunoid LOCK %s %s: best +%.2fR, now locking %s (+%.2fR)", buy ? "buy" : "sell", sym, pkR,
                  frac <= 0 ? "entry + costs — it can no longer lose" : StringFormat("%.0f%% of the best profit", frac*100.0), lock/r0);
     }
   if(TimeCurrent() - (datetime)GVget("cl_lf_"+T, 0) < 10) return;  // the broker just refused: try again in 10 s, never every second
   ...
   if(p <= lock)                                                   // back at the line: bank it now, do not wait for the stop
     {
      if(g_trade.PositionClose(tk))
         PrintFormat("Clunoid EXIT %s %s at +%.2fR — profit lock (best +%.2fR); a winner stays a winner", buy ? "buy" : "sell", sym, p/r0, pkR);
      else GVset("cl_lf_"+T, (double)TimeCurrent());
      return;
     }
   ...
   if(!g_trade.PositionModify(tk, target, tp)) GVset("cl_lf_"+T, (double)TimeCurrent());
  }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ...
      int tf = (int)GVget("cl_tf_"+T, (double)PERIOD_M15);
      ...
      double r0 = GVget("cl_r0_"+T, 0); if(r0 <= 0) r0 = 1.8*o.atr;
      string why = "";
      ...
      else if(ageBars >= 20 && GVget("cl_pk_"+T, 0) < 0.3*r0 && (o.dir != dir || TfScore(o) <= 0)) why = "the trend faded; the slot goes to a stronger market";
      if(why == "") continue;
      if(!PositionSelectByTicket(tk)) continue;
      if(g_trade.PositionClose(tk))
         PrintFormat("Clunoid EXIT %s %s at %+.2fR — %s (%s, before its stop or target)", dir > 0 ? "buy" : "sell", sym, p/r0, why, TfName(tf));
     }
  }
...
void CleanupLockGVs()
  {
   string pre[6] = {"cl_pk_", "cl_r0_", "cl_tf_", "cl_cm_", "cl_lk_", "cl_lf_"};
   for(int i=GlobalVariablesTotal()-1; i>=0; i--)
     {
      string name = GlobalVariableName(i);
      for(int k=0; k<6; k++)
...
string RsvPrefix() { return "cl_rsv"+(string)InpMagic+"_"; }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ...
      if(tk==0 || PositionGetInteger(POSITION_MAGIC)!=InpMagic) continue;
      string T = (string)tk;
      if(GlobalVariableCheck("cl_trail_"+T)) continue;                                  // it has its plan
      string cmt = PositionGetString(POSITION_COMMENT);
      if(StringFind(cmt, "clunoid-trend") < 0 && cmt != "") continue;                    // a signal-feed trade: the feed heals its own plan
      string sym = PositionGetString(POSITION_SYMBOL);
      bool buy = (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY);
      double entry = PositionGetDouble(POSITION_PRICE_OPEN), sl = PositionGetDouble(POSITION_SL), tp = PositionGetDouble(POSITION_TP);
      ...
      GVset("cl_tr_"+T, 1); GVset("cl_r0_"+T, dist); GVset("cl_tf_"+T, (double)tf);
      PrintFormat("Clunoid: adopted %s %s %.2f lots (ticket %s) — its order's answer never came back; it is now counted and managed", s.side, sym, vol, T);
     }
  }
...
string InstKey() { return "cl_inst"+(string)InpMagic+AcctSuffix(); }
...
void SayIdle()
  {
   ...
   g_saidIdle = true;
   string m = "Clunoid: this copy is IDLE — the EA already runs on another chart of this account, and one copy trades every market. Remove this copy.";
   Print(m); Comment(m);
  }
...
string TgKey() { return "cl_tg"+(string)InpMagic+AcctSuffix()+"_"; }
...
void TargetInit()
  {
   ...
   PrintFormat("Clunoid profit target: %s — counting from %s (starting balance %.2f)%s", TargetText(), TimeToString(g_tgT0), g_tgBal,
   ...
   if(!g_targetDone && goal > 0 && made >= goal)
     {
      ...
      string m = StringFormat("Clunoid: PROFIT TARGET REACHED — %.2f %s made since %s (target %s). Closing every trade and stopping.", made, cur, TimeToString(g_tgT0), TargetText());
      Print(m); Alert(m);
     }
   if(g_targetDone)
     {
...
void NoteSized(string sym, string what)
  {
   ...
   PrintFormat("Clunoid: %s — %s", sym, what);
  }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ulong tk = PositionGetTicket(i);
      if(tk==0 || PositionGetInteger(POSITION_MAGIC)!=InpMagic) continue;
      if(GVget("cl_tr_"+(string)tk, 0) > 0) n++;
     }
   return n;
  }

bool IsTrendTicket(ulong tk) { return GVget("cl_tr_"+(string)tk, 0) > 0; }
...
bool TrendEnter(TrendCand &c)
  {
   ...
   g_trade.SetTypeFillingBySymbol(sym);
   bool ok = buy ? g_trade.Buy(lots, sym, price, sl, tp, "clunoid-trend") : g_trade.Sell(lots, sym, price, sl, tp, "clunoid-trend");
   if(!ok)
     {
      uint rc = g_trade.ResultRetcode();
      PrintFormat("Clunoid trend order failed %s %s: %d %s", s.side, sym, rc, g_trade.ResultRetcodeDescription());
      StampCooldown(sym);
      ...
      if(UncertainRc(rc))
        {
         ...
         PrintFormat("Clunoid: %s got no clear answer from the broker — its slot stays held until it shows up (10 minutes at most)", sym);
        }
      return false;
     }
   ...
   if(tk > 0) { GVset("cl_tr_"+(string)tk, 1); GVset("cl_tf_"+(string)tk, (double)f.tf); GVset("cl_r0_"+(string)tk, stopDist); }
   PrintFormat("Clunoid TREND %s %s %s lots @ %s SL %s TP %s (%s · score %.0f · fit %.2f · ADX %.0f · risk %.1f%% of balance)", s.side, sym, DoubleToString(lots, 2),
...
void NoteSkip(string sym, string why)
  {
   string k = "cl_skip_"+sym;
   if(GlobalVariableCheck(k) && (TimeCurrent()-(datetime)GlobalVariableGet(k)) < 1800) return;
   GlobalVariableSet(k, (double)TimeCurrent());
   PrintFormat("Clunoid: skip %s — %s", sym, why);
  }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ulong tk = PositionGetTicket(i);
      if(tk==0 || PositionGetInteger(POSITION_MAGIC)!=InpMagic) continue;
      total += GVget("cl_risk_"+(string)tk, 0);
     }
   return total;
  }
      ...
      if(TimeCurrent() - lastSaid >= 14400)
        {
         ...
         if(err==4014 || err==4060) PrintFormat("Signal feed off: add https://www.clunoid.com in Tools > Options > Expert Advisors > WebRequest to receive it. The trend scanner keeps trading on its own.");
