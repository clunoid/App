...
/** Both kinds of Deriv id are numeric and short; anything else is a typo or a probe. */
const looksLikeId = (v: string) => /^[0-9]{4,12}$/.test(v);
...
export async function POST(req: NextRequest) {
  ...
  if (!looksLikeId(mt5Login)) {
    return NextResponse.json({ error: "That does not look like a client ID or MT5 ID — it is the number Deriv shows for the account, usually 6 to 9 digits." }, { status: 422 });
  }
