/**
 * Shared library for the per-pair strategy search + validation.
 * Indicator engine, strategy families, and the single-pair event backtest.
 */
...
export function makeInd(bars) {
  ...
  const tr = memo("tr", () => { const a = new Float64Array(N); for (let i = 0; i < N; i++) a[i] = i === 0 ? h[i] - l[i] : Math.max(h[i] - l[i], Math.abs(h[i] - c[i - 1]), Math.abs(l[i] - c[i - 1])); return a; });
  const atr = (n) => memo("atr" + n, () => { const out = new Float64Array(N).fill(NaN); let a = 0; for (let i = 1; i <= n && i < N; i++) a += tr[i]; if (N > n) { a /= n; out[n] = a; for (let i = n + 1; i < N; i++) { a = (a * (n - 1) + tr[i]) / n; out[i] = a; } } return out; });
  const ema = (n) => memo("ema" + n, () => { const out = new Float64Array(N), k = 2 / (n + 1); let p = c[0] || 0; for (let i = 0; i < N; i++) { p = i === 0 ? c[0] : c[i] * k + p * (1 - k); out[i] = p; } return out; });
  const sma = (n) => memo("sma" + n, () => { const out = new Float64Array(N).fill(NaN); let s = 0; for (let i = 0; i < N; i++) { s += c[i]; if (i >= n) s -= c[i - n]; if (i >= n - 1) out[i] = s / n; } return out; });
  const sd = (n) => memo("sd" + n, () => { const out = new Float64Array(N).fill(NaN); let s = 0, s2 = 0; for (let i = 0; i < N; i++) { s += c[i]; s2 += c[i] * c[i]; if (i >= n) { s -= c[i - n]; s2 -= c[i - n] * c[i - n]; } if (i >= n - 1) { const m = s / n; out[i] = Math.sqrt(Math.max(0, s2 / n - m * m)); } } return out; });
  const rsi = (n) => memo("rsi" + n, () => { const out = new Float64Array(N).fill(NaN); let ag = 0, al = 0; for (let i = 1; i <= n && i < N; i++) { const d = c[i] - c[i - 1]; if (d >= 0) ag += d; else al -= d; } if (N > n) { ag /= n; al /= n; out[n] = al === 0 ? 100 : 100 - 100 / (1 + ag / al); for (let i = n + 1; i < N; i++) { const d = c[i] - c[i - 1]; ag = (ag * (n - 1) + (d > 0 ? d : 0)) / n; al = (al * (n - 1) + (d < 0 ? -d : 0)) / n; out[i] = al === 0 ? 100 : 100 - 100 / (1 + ag / al); } } return out; });
  const roll = (n, isMax) => memo((isMax ? "dHi" : "dLo") + n, () => { const src = isMax ? h : l, out = new Float64Array(N).fill(NaN), dq = []; for (let i = 0; i < N; i++) { if (i >= 1) { const j = i - 1; while (dq.length && (isMax ? src[dq[dq.length - 1]] <= src[j] : src[dq[dq.length - 1]] >= src[j])) dq.pop(); dq.push(j); } while (dq.length && dq[0] < i - n) dq.shift(); if (i >= n) out[i] = src[dq[0]]; } return out; });
  const adx = (n) => memo("adx" + n, () => { const out = new Float64Array(N).fill(NaN), pDM = new Float64Array(N), mDM = new Float64Array(N); for (let i = 1; i < N; i++) { const up = h[i] - h[i - 1], dn = l[i - 1] - l[i]; pDM[i] = up > dn && up > 0 ? up : 0; mDM[i] = dn > up && dn > 0 ? dn : 0; } let trS = 0, pS = 0, mS = 0; for (let i = 1; i <= n && i < N; i++) { trS += tr[i]; pS += pDM[i]; mS += mDM[i]; } const dx = new Float64Array(N).fill(NaN); for (let i = n; i < N; i++) { if (i > n) { trS = trS - trS / n + tr[i]; pS = pS - pS / n + pDM[i]; mS = mS - mS / n + mDM[i]; } if (trS > 0) { const pdi = 100 * pS / trS, mdi = 100 * mS / trS, sum = pdi + mdi; dx[i] = sum === 0 ? 0 : 100 * Math.abs(pdi - mdi) / sum; } else dx[i] = 0; } let a = 0, cnt = 0; for (let i = n; i < Math.min(2 * n, N); i++) { a += dx[i]; cnt++; } if (N > 2 * n - 1 && cnt === n) { a /= n; out[2 * n - 1] = a; for (let i = 2 * n; i < N; i++) { a = (a * (n - 1) + dx[i]) / n; out[i] = a; } } return out; });
  return { N, t, o, h, l, c, atr, ema, sma, sd, rsi, adx, dHi: (n) => roll(n, true), dLo: (n) => roll(n, false) };
}

export function cartesian(obj) {
...
export const STRATS = {
  ...
  orb: {
    ...
    gen: (S, i, p) => {
      ...
      if (hr < p.startH || hr >= p.startH + 3) return null;
...
export function backtest(S, strat, p, cost, from, to, opt = {}) {
  const { spread, slip } = cost;
  const maxBars = opt.maxBars ?? 96;
  const news = opt.news ?? false;
  const orbDay = new Map();
  ...
  for (let i = 60; i < S.N - 1; i++) {
    ...
    if (pos) {
      ...
      if (hitSL) { exit = pos.sl; reason = "sl"; }
    ...
    if (!pos && tm >= from && tm <= to) {
      ...
      if (!sig) continue;
      if (sig.once) { if (orbDay.has(Math.floor(tm / 86400))) continue; orbDay.set(Math.floor(tm / 86400), 1); }
      const fill = S.o[i + 1];
  ...
  return {
    net: (bal - 10000) / 100, trades: trades.length, tpd: trades.length / Math.max(1, spanDays * 5 / 7),
