 ...
 *
 * The chosen profile is still saved to localStorage, because this bot's cloud
 * signal feed is generated per profile.
 */
...
import type { RiskProfile } from "@/lib/deriv/mt5/types";
...
export function GeneralMt5() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem(PROFILE_KEY) as RiskProfile | null;
  ...
  const active = PROFILE_LIST.find((p) => p.key === profile)!;
